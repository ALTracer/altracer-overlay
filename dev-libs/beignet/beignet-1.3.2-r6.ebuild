...
COMMON="
	...
	ocl-icd? ( || ( dev-libs/ocl-icd dev-libs/opencl-icd-loader ) )
...
PATCHES=(
	...
	"${FILESDIR}"/${PN}-1.3.2_llvm8-rotates.patch
...
src_prepare() {
	...
	use test || cmake_comment_add_subdirectory utests
}
...
multilib_src_configure() {
	...
	use test && local mycmakeargs=( utest )
...
multilib_src_test() {
	OCL_KERNEL_PATH="${S}"/kernels

	cmake-utils_src_test utest
}
